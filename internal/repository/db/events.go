...
func (r *DBRepository) ListEvents(ctx context.Context) ([]dbEntities.Event, error) {
	...
	err := r.db.Select(ctx, nil, &events,
		`SELECT id, title, description, date, location, type, status, discipline, image_url, images, created_at
		 FROM events
		 ORDER BY date DESC`,
	)
	if err != nil {
...
func (r *DBRepository) CreateEvent(ctx context.Context, e *dbEntities.Event) error {
	...
	err := r.db.QueryRow(ctx, nil,
		...
		`INSERT INTO events (title, description, date, location, type, status, discipline, image_url, images, created_at)
		 ...
		 RETURNING id`,
		e.Title, e.Description, e.Date, e.Location, e.Type, e.Status, e.Discipline, e.ImageURL, e.Images, time.Now(),
	)
	if err != nil {
...
func (r *DBRepository) UpdateEvent(ctx context.Context, e *dbEntities.Event) error {
	...
	_, err := r.db.Exec(ctx, nil,
		`UPDATE events
		 SET title=$1, description=$2, date=$3, location=$4, type=$5, status=$6, discipline=$7, image_url=$8, images=$9
		 WHERE id=$10`,
		e.Title, e.Description, e.Date, e.Location, e.Type, e.Status, e.Discipline, e.ImageURL, e.Images, e.ID,
	)
	if err != nil {
